...
export default class Move implements Tool {
  ...
  onOut (event) {
    const canvasContext = event.target.getContext('2d')
    const scale = 1  //todo: eliminate this when using sockets
    ...
    if (this.movingLine != null && this.currentFigureMoving instanceof Figure) {
      ...
      this.grid.moveFigure(this.currentFigureMoving, offsetPoint, canvasContext, scale) //todo: eliminate this when using sockets
