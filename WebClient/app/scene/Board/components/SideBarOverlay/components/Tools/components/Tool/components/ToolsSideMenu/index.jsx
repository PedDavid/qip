...
export default function ToolsSideMenu (props) {
  ...
  return (
    <div onMouseLeave={props.toggleSideMenu} className={styles.toolMenu} style={{width: 40 * toolContentSize, visibility}}>
      {content.map((toolInstance, idx) => (
        <div key={'toolInstance' + idx} className={styles.block} onClick={() => changeCurrentTool([toolInstance])}>
          <span>
